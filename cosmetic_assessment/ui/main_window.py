...
class MainWindow:
    ...
    def __init__(self, root: tk.Tk) -> None:
        ...
        self.current_spec_name = tk.StringVar(value="")
    ...
    def _build_layout(self) -> None:
        ...
        # ---------- Left control panel ----------
        self.sidebar = tk.Frame(self.main_frame, bg="#f3f3f3", width=260)
        self.sidebar.grid(row=0, column=0, sticky="ns", padx=(12, 6), pady=12)
    ...
    def save_inspection(self):
        ...
        export_dir = Path(folder) / f"inspection_{self.current_defect_type.get().lower()}"
    ...
    def _update_result_summary(self) -> None:
        if self.current_measurement_mm is not None:
            if self.scratch_width_mm is not None:
                self.result_var.set(
                    f"Scratch length: {self.current_measurement_mm:.2f} mm\n"
                    f"Scratch width: {self.scratch_width_mm:.2f} mm"
                )
            else:
