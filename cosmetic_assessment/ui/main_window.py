...
class MainWindow:
    ...
    def __init__(self, root: tk.Tk) -> None:
        self.root = root
        self.root.title("Cosmetic Analysis Tool")
    ...
    def _build_layout(self) -> None:
        ...
        # --- Sidebar container ---
        sidebar_container = tk.Frame(self.main_frame, bg="#f3f3f3", width=200)
        sidebar_container.grid(row=0, column=0, sticky="ns", padx=(12, 6), pady=12)
        ...
        title_lbl.pack(fill="x", pady=(10, 12), padx=10)
        ...
        self.btn_restart.pack(fill="x", padx=10, pady=(4, 6))
        ...
        self.btn_detect_fiducials.pack(fill="x", padx=10, pady=(0, 6))
        ...
        self.btn_select_roi.pack(fill="x", padx=10, pady=(0, 6))
        ...
        self.btn_scratch_detect.pack(fill="x", padx=10, pady=(0, 6))
        ...
        self.btn_save_inspection.pack(fill="x", padx=10, pady=(10, 6))
        ...
        self.image_canvas.grid(row=0, column=1, sticky="nsew", padx=(6, 12), pady=12)
    ...
    def open_image(self) -> None:
        file_path = filedialog.askopenfilename(title="Select an image", filetypes=[("Image files", "*.png *.jpg *.jpeg *.bmp *.gif *.webp"), ("All files", "*.*")])
        if not file_path:
            return
        try:
    ...
    def detect_fiducials(self) -> None:
        ...
        marker_size_mm = 30.0
        self.mm_per_px = marker_size_mm / self.detections[0].side_px if self.detections else None
        self._redraw_overlay_base()
        if self.mm_per_px is not None:
            self.status_var.set(f"Detected {len(self.detections)} fiducial marker(s). Next: choose defect type and mark the defect.")
        else:
    ...
    def _render_current_image(self) -> None:
        ...
        pil_img = Image.fromarray(rgb).resize((rendered_width, rendered_height), Image.Resampling.LANCZOS)
        self.tk_image = ImageTk.PhotoImage(pil_img)
        self.image_canvas.delete("all")
        self.canvas_image_id = self.image_canvas.create_image(self.render_offset_x, self.render_offset_y, anchor="nw", image=self.tk_image)
    ...
    def on_left_click(self, event):
        ...
        if defect == "Scratch" and self.annotation_step == "width":
            ...
            if not self._can_place_scratch_width_sample(self.scratch_width_anchor):
                self.status_var.set(
            ...
            sample = {
                "anchor": self.scratch_width_anchor,
                "p1": p1,
                "p2": p2,
                "width_px": width_px,
                "width_mm": width_px * self.mm_per_px,
            }
            ...
            self.status_var.set(
                ...
                f"Average width: {self.scratch_width_mm:.2f} mm"
            )
    ...
    def finish_annotation(self):
        ...
        if "length_mm" in data:
            self.current_measurement_mm = data["length_mm"]
        if "length_points" in data:
            self.scratch_length_points = data["length_points"]

        if "width_mm" in data:
            self.scratch_width_mm = data["width_mm"]
        if "width_points" in data:
            self.scratch_width_points = data["width_points"]

        if "area_mm2" in data:
            self.current_area_mm2 = data["area_mm2"]

        if "diameter_mm" in data:
            self.current_diameter_mm = data["diameter_mm"]

        if result.get("complete", False):
            self.annotation_active = False
        else:
            self.annotation_step = result.get("next_step")
            self.annotation_active = True
            self.measure_points = []

        self.instructions_var.set(result.get("instructions", self.instructions_var.get()))
        self.status_var.set(result.get("status", "Annotation finished"))
    ...
    def _redraw_overlay_base(self):
        ...
        if self.scratch_candidates and self.current_defect_type.get() == "Scratch":
            x1, y1, _, _ = self.roi_rect
            for i, cnt in enumerate(self.scratch_candidates):
                ...
                cv2.drawContours(base, [cnt_shifted], -1, color, 2 if i else 3)
        self._draw_annotation(base)
        self.image_state.display_bgr = base
        self._render_current_image()

    def _draw_annotation(self, base):
        is_scratch = self.current_defect_type.get() == "Scratch"

        # Draw active scratch centerline while annotating length
        if is_scratch and self.annotation_step == "length" and self.measure_points:
            self._current_mode().draw(base, self.measure_points, self.annotation_active)

        # Draw saved scratch centerline after length is finished
        if is_scratch and len(self.scratch_length_points) >= 2:
            pts = np.array(self.scratch_length_points, dtype=np.int32)
            cv2.polylines(base, [pts], False, (0, 255, 255), 2)

        # Draw saved fishbone ribs
        if is_scratch:
            for sample in self.scratch_width_samples:
                cv2.line(base, sample["p1"], sample["p2"], (0, 255, 255), 1)
                ax, ay = sample["anchor"]
                cv2.circle(base, (ax, ay), 3, (255, 255, 0), -1)

        # Draw live preview rib in width mode
        if (
            is_scratch
            and self.annotation_step == "width"
            and self.scratch_width_preview is not None
        ):
            p1 = self.scratch_width_preview["p1"]
            p2 = self.scratch_width_preview["p2"]
            ...
            if self.scratch_width_anchor is not None:
                ax, ay = self.scratch_width_anchor
                cv2.circle(base, (int(ax), int(ay)), 4, color, -1)

        # Other defect modes still use generic drawing
    ...
    def save_inspection(self):
        ...
        folder = filedialog.askdirectory(title="Select folder to save inspection")
        if not folder:
            return

        export_dir = self.build_inspection_folder(folder, self.current_defect_type.get())
        ...
        cv2.imwrite(str(original_path), self.image_state.original_bgr)
    ...
    def on_mouse_move(self, event):
        ...
        if (
            ...
            self.scratch_width_anchor = anchor
    ...
    def finish_scratch_width(self):
        ...
        self.status_var.set(
            ...
            f"Avg width: {avg_width:.2f} mm | "
            f"Min: {min_width:.2f} mm | Max: {max_width:.2f} mm"
        )
    ...
    def _can_place_scratch_width_sample(self, anchor):
        ...
        min_spacing_px = 12
        # optional physical spacing version:
        # min_spacing_px = 10 / self.mm_per_px if self.mm_per_px else 12

        return self._distance(anchor, last_anchor) >= min_spacing_px
